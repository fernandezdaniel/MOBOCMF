...
class Training_Dataset:
    def __init__(self, inputs, targets, verbose=True): # ARREGLAR ESTA FUNCION PARA QUE DEJE DE TRANSFORMAR LOS INPUTS Y CENTRARLOS EN 0.5
        self.inputs = inputs
...
class StepFunction_Dataset(DGPDataset):
    def __init__(self, num_samples=1000, seed=0):
        rng = np.random.default_rng(seed=seed)

        def sign(x):
            return x >= 0.0

        inputs = rng.uniform(low=-1.0, high=1.0, size=num_samples)
        targets = sign(inputs) + rng.standard_normal(inputs.shape) * 0.1

        self.inputs = inputs[..., np.newaxis]
        self.targets = targets[..., np.newaxis]


class Synthetic_Dataset(DGPDataset):
    def __init__(self, num_samples=300, seed=0):
        rng = np.random.default_rng(seed=seed)

        def f(x):
            return np.cos(5 * x) / (np.abs(x) + 1)

        inputs = rng.standard_normal(num_samples)
...
class Boston_Dataset(DGPDataset):
    def __init__(self):
...
class Energy_Dataset(DGPDataset):
    def __init__(self):
...
class Concrete_Dataset(DGPDataset):
    def __init__(self):
...
class Naval_Dataset(DGPDataset):
    def __init__(self):
...
class Power_Dataset(DGPDataset):
    def __init__(self):
...
class Protein_Dataset(DGPDataset):
    def __init__(self):
...
class Kin8nm_Dataset(DGPDataset):
    def __init__(self):
        url = (
...
class Yatch_Dataset(DGPDataset):
    def __init__(self):
...
class WineRed_Dataset(DGPDataset):
    def __init__(self):
...
class WineWhite_Dataset(DGPDataset):
    def __init__(self):
...
def get_dataset(dataset_name):
    ...
    elif dataset_name == "step_function":
        return StepFunction_Dataset()
    else:
