...
class BlackBoxMFDGPFitter():
    ...
    def _train_mfdgp_and_plot(self, func_update_model, fix_variational_hypers, num_epochs, lr,
                              axs,
        ...
        for (handler_con, optimizer, n_con) in zip(self.mfdgp_handlers_cons.values(), l_opt_cons, np.arange(len(self.mfdgp_handlers_cons))):
            ...
            for i in range(num_epochs):
                ...
                if i % step_plot == 0:
                    ...
                    if len(self.iters_steps) == 0:
                        self.iters_steps.append(i)
                    else:
                        self.iters_steps.append(self.iters_steps[-1] + step_plot)

                    self.loss_iters.append(loss_iter.item())
                    self.nelbo_iters.append(loss_iter.item() - kl_iter.item())
                    self.kl_iters.append(kl_iter.item())

                    x_func = np.linspace(lower_limit, upper_limit, 1000)[:, None]
                    spacing = torch.linspace(lower_limit, upper_limit, 200).double()[ : , None ]

                    handler_con.mfdgp.eval()
                    con1_pred_mean_mf0, con1_pred_std_mf0 = compute_moments_mfdgp(handler_con.mfdgp, spacing, mean=con1_mean_mf0, std=con1_std_mf0, fidelity=0)
                    con1_pred_mean_mf1, con1_pred_std_mf1 = compute_moments_mfdgp(handler_con.mfdgp, spacing, mean=con1_mean_mf1, std=con1_std_mf1, fidelity=1)
                    handler_con.mfdgp.train()

                    # Actualizar el plot principal
                    ax0.cla()
                    plot_model(ax0, spacing.numpy(),
                               x_func, x_func,
                               func_con1_mf0(x_func), func_con1_mf1(x_func),
                               x_mf0, x_mf1,
                               con1_mean_mf0, con1_mean_mf1,
                               con1_std_mf0, con1_std_mf1,
                               con1_train_mf0, con1_train_mf1,
                    ...
                    ax1.cla()
                    ax1.plot(self.iters_steps, self.loss_iters, 'b-', label="Loss")
                    ax1.plot(self.iters_steps, self.nelbo_iters, 'g-', label="NELBO")
                    ax1.plot(self.iters_steps, self.kl_iters, 'r-', label="KL")
                    ax1.legend()
                    ...
                    ax2.cla()
                    ax2.plot(self.iters_steps, self.vals_param_k_x_1_outputscale, 'b-')
                    ax2.set_title('k_x_1 outputscale')

                    ax3.cla()
                    ax3.plot(self.iters_steps, self.vals_param_k_x_1_lengthscale, 'b-')
                    ax3.set_title('k_x_1 lengthscale')
                    
                    ax4.cla()
                    ax4.plot(self.iters_steps, self.vals_param_k_lin_variance, 'b-')
                    ax4.set_title('k_lin variance')
                    
                    ax5.cla()
                    ax5.plot(self.iters_steps, self.vals_param_k_f_outputscale, 'b-')
                    ax5.set_title('k_f outputscale')
                    
                    ax6.cla()
                    ax6.plot(self.iters_steps, self.vals_param_k_f_lengthscale, 'b-')
                    ax6.set_title('k_f lengthscale')
                    
                    ax7.cla()
                    ax7.plot(self.iters_steps, self.vals_param_k_x_2_outputscale, 'b-')
                    ax7.set_title('k_x_2 outputscale')
                    
                    ax8.cla()
                    ax8.plot(self.iters_steps, self.vals_param_k_x_2_lengthscale, 'b-')
                    ax8.set_title('k_x_2 lengthscale')
                    
                    ax9.cla()
                    ax9.plot(self.iters_steps, self.vals_param_hidden_layer_likelihood_1_noise, 'b-')
                    ax9.set_title('hidden_layer_likelihood_1 noise')

                    plt.suptitle("Iter: " + str(self.iters_steps[-1]))
                    plt.pause(0.3)

                    if len(self.iters_steps) == 1: input()

    def train_mfdgps_and_plot(self,
                              lower_limit, upper_limit,
                              func_con1_mf0, func_con1_mf1,
                              x_mf0, x_mf1,
                              con1_mean_mf0, con1_mean_mf1,
                              con1_std_mf0, con1_std_mf1,
                              con1_train_mf0, con1_train_mf1, steps_plot):
        ...
        self.loss_iters, self.nelbo_iters, self.kl_iters = [], [], []
        ...
        self._train_mfdgp_and_plot(_update_model,
                                   ...
                                   func_con1_mf0=func_con1_mf0, func_con1_mf1=func_con1_mf1,
                                   x_mf0=x_mf0, x_mf1=x_mf1,
                                   con1_mean_mf0=con1_mean_mf0, con1_mean_mf1=con1_mean_mf1,
                                   con1_std_mf0=con1_std_mf0, con1_std_mf1=con1_std_mf1,
                                   con1_train_mf0=con1_train_mf0, con1_train_mf1=con1_train_mf1,
                                   step_plot=steps_plot)
        self._train_mfdgp_and_plot(_update_model,
