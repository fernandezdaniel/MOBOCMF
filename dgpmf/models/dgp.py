...
class DGP_Base(tf.Module):
    def __init__(
        ...
        **kwargs : dict, optional
                   ...
                   Refer to tf.keras.Model for more information.
        """
        super().__init__(name="DGP_Base", **kwargs)
    ...
    def nelbo(self, inputs, outputs, iteration=None):
        ...
        """
        X, Y = inputs, outputs
        likelihood = tf.reduce_sum(self.E_log_p_Y(X, Y))
        ...
        # Compute KL term
        KL = tf.reduce_sum([layer.KL() for layer in self.gp_layers])
