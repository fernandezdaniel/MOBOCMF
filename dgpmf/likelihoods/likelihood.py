...
class Gaussian(Likelihood):
    ...
    def variational_expectations(self, Fmu, Fvar, Y): # _res = -0.5 * (np.log(2 * np.pi) + self.log_variance + ((np.array(Fmu) - np.array(Y))**2 + Fvar) / np.exp(self.log_variance))
        ...
        res = tf.reduce_sum(var_exp, -1)

        if np.any(np.isnan(np.array(res))):
            print("stop")
