...
def high_fidelity_obj1(x):
    return np.sum(x**2, 1)

def low_fidelity_obj1(x):
    return 2.0 * np.sum(x**2, 1) + 0.1 * np.sum(x, 1)

def high_fidelity_obj2(x):
    return -1.0 * np.sum(x**2, 1)

def low_fidelity_obj2(x):
    return -1.0 * (2.0 * np.sum(x**2, 1) + 0.1 * np.sum(x, 1))

def high_fidelity_con1(x):
    return np.sin(4 * np.sum(x, 1))
    
def low_fidelity_con1(x):
    return np.cos(4 * np.sum(x, 1))

def high_fidelity_con2(x):
    return np.sum(x**2, 1) - 0.5
    
def low_fidelity_con2(x):
    return 3.0 * (np.sum(x**2, 1) - 0.5) + 0.1 * np.sum(x, 1)
...
#with open("blackbox_mfdgp_fitters_2D/mfdgp_uncond_%dxmf0_%dxmf1_%d_%d.dat"% (num_inputs_low_fidelity, num_inputs_high_fidelity, num_epochs_1, num_epochs_2), "wb") as fw:
#    pickle.dump(blackbox_mfdgp_fitter, fw)

with open("blackbox_mfdgp_fitters_2D/mfdgp_uncond_%dxmf0_%dxmf1_%d_%d.dat"% (num_inputs_low_fidelity, num_inputs_high_fidelity, num_epochs_1, num_epochs_2), "rb") as fr:
