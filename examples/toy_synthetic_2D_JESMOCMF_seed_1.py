...
def get_valid_synthetic_constraints(mfdgp_con1, mfdgp_con2, grid, prob_valid=0.5, num_tries=50):
    ...
    for i in range(num_tries):
        ...
        if np.mean((high_fidelity_con1_vals >= 0.0) * (high_fidelity_con2_vals >= 0.0)) > prob_valid:
...
low_fidelity_con1, high_fidelity_con1, low_fidelity_con2, high_fidelity_con2 = \
    get_valid_synthetic_constraints(con1, con2, numpy_grid_for_optimal_hv, prob_valid=0.5, num_tries=50)
...
#        previously_trained_model = blackbox_mfdgp_fitter_previous_iteration.mfdgp_handlers_cons[ "con2" ].mfdgp
    ...
    if load_models_form_disk == False:
...
#    # XXX DHL We resotre the mean and variance:
    ...
    if iteration == 0:
        with open('hypervolumes.txt', 'w') as f:
        ...
        with open('hypervolume_solution.txt', 'w') as f:
            print("%lf" % (optimal_hv_iter), file=f)

        with open('fidelities_evaluated.txt', 'w') as f:
